...
def corruptImg(img, s, plot=False):    
    ...
    for x in range(0,img.shape[0],8):
        for y in range(0,img.shape[1],K):
            ...
            corrupted_img[x:x+K,y:y+K] = createSnChip(chip, s)
    if plot:
        fig,ax = plt.subplots()
...
def reconstruct_chip(corrupted_chip, m, plot=False):
...
def reconstruct_img(corrupted_img, m):  
    ...
    for x in range(0,corrupted_img.shape[0],K):
        for y in range(0,corrupted_img.shape[1],K):
            ...
            pred_chip = reconstruct_chip(corrupted_chip, m)
            reconstructed_img[x:x+K,y:y+K] = pred_chip
    
    return reconstructed_img

def simulate_reconstruct_img(img, s, plot=False):
    print("s:",s)
    corrupted_img = corruptImg(img, s, plot=plot)
    reconstructed_img = reconstruct_img(corrupted_img, s//6)
